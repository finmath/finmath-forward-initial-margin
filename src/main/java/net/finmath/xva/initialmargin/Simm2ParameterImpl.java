...
public final class Simm2ParameterImpl implements Simm2Parameter {
	private final String RESIDUAL_BUCKET = "Residual";
	private final double[] EQUITY_INTRA_BUCKET_CORRELATIONS = {0.14, 0.2, 0.19, 0.21, 0.24, 0.35, 0.34, 0.34, 0.2, 0.24, 0.62, 0.62, 0.0};
	private final double[][] EQUITY_CROSS_BUCKET_CORRELATIONS = {
	...
	private final double[] EQUITY_DELTA_RISK_WEIGHTS = {25.0, 32.0, 29.0, 27.0, 18.0, 21.0, 25.0, 22.0, 27.0, 29.0, 16.0, 16.0, 32.0};
	private final double[] EQUITY_DELTA_THRESHOLDS = {3300000.0, 3300000.0, 3300000.0, 3300000.0, 3.0E7, 3.0E7, 3.0E7, 3.0E7, 600000.0, 2300000.0, 9.0E8, 9.0E8, 600000.0};
	private final double[] COMMODITY_INTRA_BUCKET_CORRELATIONS = {0.3, 0.97, 0.93, 0.98, 0.99, 0.92, 1.0, 0.58, 1.0, 0.1, 0.55, 0.64, 0.71, 0.22, 0.29, 0.0, 0.21};
	private final double[][] COMMODITY_CROSS_BUCKET_CORRELATIONS = {
	...
	private final double[] COMMODITY_DELTA_RISK_WEIGHTS = {19.0, 20.0, 17.0, 18.0, 24.0, 20.0, 24.0, 41.0, 25.0, 91.0, 20.0, 19.0, 16.0, 15.0, 10.0, 91.0, 17.0};
	private final double[] COMMODITY_DELTA_THRESHOLDS = {1.4E9, 2.0E10, 3.5E9, 3.5E9, 3.5E9, 6.4E9, 6.4E9, 2.5E9, 2.5E9, 3.0E8, 2.9E9, 7.6E9, 3.9E9, 3.9E9, 3.9E9, 3.0E8, 1.2E10};
	private final double[] CREDIT_Q_DELTA_RISK_WEIGHTS = {85.0, 85.0, 73.0, 49.0, 48.0, 43.0, 161.0, 238.0, 151.0, 210.0, 141.0, 102.0, 238.0};
	private final double[] CREDIT_Q_DELTA_THRESHOLDS = {950000.0,290000.0,290000.0,290000.0,290000.0,290000.0,950000.0,290000.0,290000.0,290000.0,290000.0,290000.0,290000.0};
	private final double[][] CREDIT_Q_CROSS_BUCKET_CORRELATIONS = {
	...
	private final double[] CREDIT_NON_Q_DELTA_RISK_WEIGHTS = {140.0, 2000.0, 2000.0};
	private final double[] CREDIT_NON_Q_DELTA_THRESHOLDS = {9500000.0,500000.0,500000.0};
	private final Set<String> FX_CATEGORY_1 = new HashSet<>(Arrays.asList("USD", "EUR", "JPY", "GBP", "AUD", "CHF", "CAD"));
	private final Set<String> FX_CATEGORY_2 = new HashSet<>(Arrays.asList("BRL", "CNY", "HKD", "INR", "KRW", "MXN", "NOK", "NZD", "RUB", "SEK", "SGD", "TRY", "ZAR"));
	...
	private double getDeltaConcentrationThreshold(Simm2Coordinate coordinate) {
		switch (coordinate.getRiskClass()) {
			case FX:
				if (FX_CATEGORY_1.contains(coordinate.getRiskFactorKey().toUpperCase())) {
					return 8400E6;
				}
				if (FX_CATEGORY_2.contains(coordinate.getRiskFactorKey().toUpperCase())) {
					return 1900E6;
				}
				return 560E6;
			case EQUITY:
			...
			default:
				throw new UnsupportedOperationException("Delta concenctration threshold for IR not available yet.");
		}
	}
