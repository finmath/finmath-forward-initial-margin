...
public class SimmNonIRScheme extends SimmBaseScheme {
	 ...
	 */
	public WeightedSensitivity getWeightedSensitivity(SimmCoordinate coordinate, RandomVariable x) {
		...
		RandomVariable a = x.mult(parameter.getAdditionalWeight(coordinate));
		...
		final RandomVariable concentrationRiskFactor = a.abs().div(threshold).sqrt().cap(1.0);
	...
	protected Stream<Map.Entry<SimmCoordinate, RandomVariable>> streamGradient(Map<SimmCoordinate, RandomVariable> gradient) {
		return gradient.entrySet().stream().
				...
				map(group -> Pair.of(
						group.getKey(),
						group.getValue().stream().map(Pair::getValue).reduce(new Scalar(0.0), RandomVariable::add)
						));
	}
}
