...
public class SIMMProductFigures {
	...
	public double[] getExpectedIM() throws CalculationException{
		...
		for(int i=0;i<expectedIM.length;i++) {
			expectedIM[i] = forwardIM[i].getAverage();
		}
		return expectedIM;
	}
	...
	public double[] getQuantile(double quantile) throws CalculationException{
		...
		for(int i=0;i<quantileIM.length;i++) {
			quantileIM[i] = forwardIM[i].getQuantile(quantile);
		}
		return quantileIM;
	}
	...
	public double[] getPath(int pathIndex) throws CalculationException{
		...
		for(int i=0;i<pathIM.length;i++) {
			pathIM[i] = forwardIM[i].get(pathIndex);
		}
		return pathIM;
	}
